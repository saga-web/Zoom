...
def main():
    ...
    file_count, total_size, skipped_count = download_recordings(get_username_input(), from_date, to_date)
...
def fetch_token():
    ...
    response = requests.post('https://api.zoom.us/oauth/token', auth=(CONFIG.CLIENT_ID, CONFIG.CLIENT_SECRET),
                             data=data).json()
...
def download_recordings(users, from_date, to_date):
    ...
    for user_email, user_name in users:
        ...
        user_file_count, user_total_size, user_skipped_count = download_recordings_from_meetings(meetings,
                                                                                                 user_host_folder)
...
def download_recordings_from_meetings(meetings, host_folder):
    ...
    for meeting in meetings:
        if CONFIG.TOPICS and meeting['topic'] not in CONFIG.TOPICS and utils.slugify(
                meeting['topic']) not in CONFIG.TOPICS:
            continue

        recording_files = meeting.get('recording_files') or []
        participant_audio_files = meeting.get(
            'participant_audio_files') or [] if CONFIG.INCLUDE_PARTICIPANT_AUDIO else []
        ...
        for recording_file in recording_files + participant_audio_files:
            ...
            file_name_suffix = os.path.splitext(recording_file['file_name'])[
                                   0] + '__' if 'file_name' in recording_file else ''
            recording_type_suffix = recording_file[
                                        "recording_type"] + '__' if 'recording_type' in recording_file else ''
